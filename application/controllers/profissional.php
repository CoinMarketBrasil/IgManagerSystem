...
class profissional extends CI_Controller
{
	...
	function manage_patient($param1 = '', $param2 = '', $param3 = '')
	{
		...
		if ($param1 == 'edit' && $param2 == 'do_update') {
			...
			$data['age']         = $this->input->post('age');
			$data['TEA']         = $this->input->post('TEA');
			$data['TEA_OBS']     = $this->input->post('TEA_OBS');
			$data['LDC']         = $this->input->post('LDC');
			$data['UDC']         = $this->input->post('UDC');
			$data['EAS']         = $this->input->post('EAS');
			$data['TBG']         = $this->input->post('TBG');
			$data['IBA']         = $this->input->post('IBA');
			$data['FI']         = $this->input->post('FI');
			$data['QDS']         = $this->input->post('QDS');
			$data['IDA']         = $this->input->post('IDA');
			$data['PAF']         = $this->input->post('PAF');
			$data['TMA']         = $this->input->post('TMA');
			$data['TMA_OBS']         = $this->input->post('TMA_OBS');
			$data['ALRG']         = $this->input->post('ALRG');
			$data['PDMP']         = $this->input->post('PDMP');
			$data['ATCD']         = $this->input->post('ATCD');
			$data['ATCD_OBS']         = $this->input->post('ATCD_OBS');
			$data['HIPAR']         = $this->input->post('HIPAR');
			$data['DISCIR']         = $this->input->post('DISCIR');
			$data['DISRE']         = $this->input->post('DISRE');
			$data['DISRE_OBS']         = $this->input->post('DISRE_OBS');
			$data['DISHOR']         = $this->input->post('DISHOR');
			$data['DISHOR_OBS']         = $this->input->post('DISHOR_OBS');
			$data['DIABT']         = $this->input->post('DIABT');
			$data['ACNE']         = $this->input->post('ACNE');
			$data['MANCHPIG']         = $this->input->post('MANCHPIG');
			$data['CORPEL']         = $this->input->post('CORPEL');
			$data['blood_group'] = $this->input->post('blood_group');
	...
	/******ALLOT / DISCHARGE allotments TO PATIENTS*****/
	function manage_consult_allotment($param1 = '', $param2 = '', $param3 = '')
	{
		if ($this->session->userdata('profissional_login') != 1)
			redirect(base_url() . 'index.php?login', 'refresh');
		
		//create a new allotment only in available / 
		if ($param1 == 'create') {
