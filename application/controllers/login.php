...
class Login extends CI_Controller
{
	...
	function _validate_login($str)
	{
		...
		if ($query->num_rows() > 0) {
			...
			if ($this->input->post('login_type') == 'profissional') {
				$this->session->set_userdata('login_type', 'profissional');
				$this->session->set_userdata('profissional_login', '1');
				$this->session->set_userdata('profissional_id', $row->profissional_id);
			}
	...
	/***LOGIN AS ANOTHER USER LIKE profissional,PATIENT,PHARMACIST,LABORATORIST ETC******/
