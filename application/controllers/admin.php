...
class Admin extends CI_Controller
{
	...
	/***especialidadeS OF profissionalS********/
	function manage_especialidade($param1 = '', $param2 = '', $param3 = '')
	{
		if ($this->session->userdata('admin_login') != 1)
			redirect(base_url() . 'index.php?login', 'refresh');
		...
		if ($param1 == 'create') {
			...
			$this->session->set_flashdata('flash_message', get_phrase('especialidade_opened'));
			redirect(base_url() . 'index.php?admin/manage_especialidade', 'refresh');
		}
		if ($param1 == 'edit' && $param2 == 'do_update') {
			$data['name']        = $this->input->post('name');
			$data['description'] = $this->input->post('description');
			$this->db->where('especialidade_id', $param3);
			$this->db->update('especialidade', $data);
			$this->session->set_flashdata('flash_message', get_phrase('especialidade_updated'));
			redirect(base_url() . 'index.php?admin/manage_especialidade', 'refresh');
			
		} else if ($param1 == 'edit') {
			$page_data['edit_profile'] = $this->db->get_where('especialidade', array(
				'especialidade_id' => $param2
			))->result_array();
		}
		if ($param1 == 'delete') {
			$this->db->where('especialidade_id', $param2);
			$this->db->delete('especialidade');
			$this->session->set_flashdata('flash_message', get_phrase('especialidade_deleted'));
			redirect(base_url() . 'index.php?admin/manage_especialidade', 'refresh');
		}
		$page_data['page_name']   = 'manage_especialidade';
		$page_data['page_title']  = get_phrase('manage_especialidade');
		$page_data['especialidades'] = $this->db->get('especialidade')->result_array();
		$this->load->view('index', $page_data);
		
	}
	/***Manage profissionals**/
	function manage_profissional($param1 = '', $param2 = '', $param3 = '')
	{
		if ($this->session->userdata('admin_login') != 1)
			redirect(base_url() . 'index.php?login', 'refresh');
		...
		if ($param1 == 'create') {
			...
			$data['especialidade_id'] = $this->input->post('especialidade_id');
			$data['profile']       = $this->input->post('profile');
			$this->db->insert('profissional', $data);
			$this->email_model->account_opening_email('profissional', $data['email']); //SEND EMAIL ACCOUNT OPENING EMAIL
			$this->session->set_flashdata('flash_message', get_phrase('account_opened'));
			
			redirect(base_url() . 'index.php?admin/manage_profissional', 'refresh');
		}
		if ($param1 == 'edit' && $param2 == 'do_update') {
			$data['name']          = $this->input->post('name');
			$data['email']         = $this->input->post('email');
			...
			$this->db->where('profissional_id', $param3);
			...
			$page_data['edit_profile'] = $this->db->get_where('profissional', array(
				'profissional_id' => $param2
			))->result_array();
		}
		if ($param1 == 'delete') {
			$this->db->where('profissional_id', $param2);
			$this->db->delete('profissional');
			$this->session->set_flashdata('flash_message', get_phrase('account_deleted'));
			
			redirect(base_url() . 'index.php?admin/manage_profissional', 'refresh');
		}
		$page_data['page_name']  = 'manage_profissional';
		$page_data['page_title'] = get_phrase('manage_profissional');
		$page_data['profissional']    = $this->db->get('profissional')->result_array();
		$this->load->view('index', $page_data);
